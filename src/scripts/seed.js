...
let superAdmin = await db.prepare('SELECT * FROM users WHERE email = ?').get(superEmail);
if (!superAdmin) {
  const passwordHash = await hashPassword('SuperAdmin123!');
  const result = await db.prepare("INSERT INTO users (name, email, password_hash, role, status) VALUES (?, ?, ?, 'super_admin', 'active')").run('Platform Super Admin', superEmail, passwordHash);
  superAdmin = await db.prepare('SELECT * FROM users WHERE id = ?').get(result.lastInsertRowid);
}
...
if (!user) {
  ...
  const result = await db.prepare("INSERT INTO users (name, email, password_hash, role, status) VALUES (?, ?, ?, 'owner', 'active')").run('Demo Owner', email, passwordHash);
  user = await db.prepare('SELECT * FROM users WHERE id = ?').get(result.lastInsertRowid);
}

let event = await db.prepare('SELECT * FROM events WHERE slug = ?').get('rahul-anjali-demo');
if (!event) {
  ...
  const folders = ['Haldi', 'Mehndi', 'Baraat', 'Reception'];
  for (let index = 0; index < folders.length; index++) {
    const name = folders[index];
    await db.prepare('INSERT INTO folders (event_id, name, sort_order) VALUES (?, ?, ?)').run(eventId, name, index);
  }
}
